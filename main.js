    ...
    try {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *𝚀𝚄𝙴𝙴𝙽 𝙰𝙺𝙸𝚁𝙰 𝚅2 — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴀᴋɪʀᴀ ᴛᴇᴄʜ*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363429835502138@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© 𝚀𝚄𝙴𝙴𝙽 𝙰𝙺𝙸𝚁𝙰 𝚅2',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:ARSLAN-MD BOY\nORG:ARSLAN-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© 𝚀𝚄𝙴𝙴𝙽 𝙰𝙺𝙸𝚁𝙰 𝚅2' } }
                    }},
        ...
        arslanLog(`𝚀𝚄𝙴𝙴𝙽 𝙰𝙺𝙸𝚁𝙰 𝚅2 Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𝚀𝚄𝙴𝙴𝙽 𝙰𝙺𝙸𝚁𝙰 𝚅2 — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
